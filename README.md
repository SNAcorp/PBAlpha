...
`Swap` - это класс, предназначенный для работы с файлом "swap.json", который используется для временного хранения информации о заказах в случае отключения сервера. Класс позволяет сохранять данные из файла "swap.json" на сервер, когда сервер снова онлайн.

### Атрибуты

- `__storage`: Экземпляр класса `Storage`, предназначенного для работы с файловой системой.
- `__file`: Путь к файлу "swap.json".
- `__server`: URL сервера.

### Методы

#### `__init__()`

Инициализирует класс `Swap`. При инициализации проверяет, существует ли файл "swap.json". Если файл существует, данные из него отправляются на сервер по одному заказу за раз с использованием HTTP PUT запроса. После успешной отправки данных файл "swap.json" удаляется.
